...
$catracaMovel = $_POST["Catracas_Moveis"];
$cordas = $_POST["cordas"];
$Cinta = $_POST["cinta"];
...
$sql = "INSERT INTO ficha_sprinter VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stm = $conexao->prepare($sql);
$stm->bind_param('issssssssssssssssssssssssssssssss', $codigo, $placa, $extintor, $capaDeChuva, $ColeteRefletivo,$Xrefletivo, $parDeluvas, $capacete, $GuardaChuva,$MarteloMadeira,$Documentos,$Acendedor,$CabosForca,$Macaco,$FerroMacaco,$Triangulo,$ChaveRodas,$tapetes,$catracas,$catracaMovel,$cordas,$Cinta,$carrinhoHidraulico,$calotas,$qtsChaves,$OculosProtecao,$Controle,$KitEmergenciaG,$KitEmergenciaP,$lampadasDianteiras,$lampadasLaterais, $lampadasTraseiras, $Interior);
